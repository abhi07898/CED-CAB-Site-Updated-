    ...
    <div class="container-fluid">       
            ...
            <?php
                    if(isset($_SESSION['cart'])){
                        ...
                        $cab_type = $_SESSION['cart']['cabdata'];
                        $obj = new rideclasses();
...
<div id="pending_rides_div">
  <div class="w-100 pt-2 pb-3  bg-dark text-light mt-2 text-center ">
    List of all the PENDING REQUEST <select class="btn btn-outline-warning" id="pend_ride_sort" class='ml-5 mr-5'>
...
<div id="complete_rides_div" class="mt-2">
  ...
  <select class="btn btn-outline-warning" id="comp_ride_sort" class='ml-5 mr-5'>
  <option selected>Select for Sort Table </option>
  <option value="total_distance">Distance</option>
  <option value="toatl_fare">Fare</option>
  <option value="ride_date">Date</option>
        ...
        $('#complete_rides').click(function(){
          function complete_rides() {
            ...
            $.ajax({
                ...
                success : function(data) {
                    output = '<table border="1" width="100%" cellspacing="0" cellpadding ="10px">';
                    output+="<tr align='center' height='40px'><th>RIDE DATE</th><th>PICKUP</th><th>DESTINATION</th><th>DISTANCE</th><th>CAB TYPE</th><th>FARE</th><th>Detailed Invoice</th></tr>";
                    // var total_spent=0;
                    // var total_ride = 0;
                    for(i=0;i<data.length;i++) {
          ...
          $(document).on('click','.envoice-btn', function(){
            ...
            var search_key = $(this).val();
            var action = 'comp_ride_search_filter';
            var id ='<?php echo $_SESSION['user']['id'];?>';
            $.ajax({
              url : 'userajaxaction.php',
              type:'POST',
